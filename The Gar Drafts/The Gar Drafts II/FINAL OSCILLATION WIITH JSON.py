...
def draw_grid1(screen, array, data):
    ...
    max_data_value = max(data)  # Find the maximum value in data for normalization
    min_data_value = min(data)  # Find the minimum value in data for normalization

    for row in range(GRID_SIZE):
        for col in range(GRID_SIZE):
            index = row * GRID_SIZE + col

            # Check if the binary_list (array) at index is 1
            if array[index] == 1:
                # Normalize the data value to 0-255 range for grayscale
                if max_data_value != min_data_value:  # Avoid division by zero
                    ...
                    normalized_value = (
                        255  # If all values are the same, default to white
                    )
                color = (normalized_value, normalized_value, normalized_value)
            else:
                color = BLACK  # Use black or any other default color for cells with a 0 in array
...
serial_output_port = "/dev/tty.usbmodem1301"
baud_rate = 115200
